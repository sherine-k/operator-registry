...
func NewMirrorFilter(config FilterConfiguration, filterOpts ...FilterOption) declcfg.CatalogFilter {
	opts := filterOptions{
		Log: nullLogger(),
	}
...
func (f *mirrorFilter) FilterCatalog(ctx context.Context, fbc *declcfg.DeclarativeConfig) (*declcfg.DeclarativeConfig, error) {
	...
	if f.opts.Full {
		return f.filterCatalogInFull(ctx, fbc)
	}
	...
	for i, fbcCh := range fbc.Channels {
		...
		if !ok {
			_, exists := slices.BinarySearchFunc(fbc.Packages, fbcCh.Package, func(pkg declcfg.Package, pkgName string) int {
				return strings.Compare(pkg.Name, pkgName) + strings.Compare(pkg.DefaultChannel, fbcCh.Name)
			})
			if exists {
